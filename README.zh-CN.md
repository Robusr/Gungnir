...
<p align="center">
  ...
  <a href="#路线图"><img src="https://img.shields.io/badge/status-M0%E2%80%93M6%20complete-brightgreen.svg" alt="Status: M0-M6 complete"></a>
</p>
...
Gungnir（冈格尼尔，北欧神话中奥丁的长矛）面向北大光华企业竞争模拟平台（[edu.ibizsim.cn](https://edu.ibizsim.cn)），是企业竞争模拟（BizSim）的 AI 决策副驾驶，面向场景 **5A**：2 产品 x 3 市场、10 家公司，1 期 = 1 季度。

## 决策闭环

Gungnir 覆盖完整的决策闭环，每一步都有人参与：

```
录入状态 -> 提案 / 优化 -> 调整 -> 解释 -> 导出
```

1. **录入**期初状态：现金、工人、机器、库存、贷款与评分历史。
2. **提案**一套可行决策，或由确定性搜索**优化**。
3. **调整**任意字段——每次修改即时重新仿真与校验。
4. 用中文**解释**决策：LLM 讲述「为什么」，只引用引擎算好的数字。
5. **导出**决策单与现金流表为 CSV，即可提交。

## 核心特性

| 特性 | 说明 |
|------|------|
| 确定性规则引擎 | 纯函数、无随机性；同一输入得到同一输出，现金流可精确对拍到分。 |
| 永远可行 | 任何提案 / 优化结果都通过硬约束校验，绝不产生不可行决策。 |
| LLM 只解释、不算术 | 数字全部由引擎算好，LLM 仅引用、不推导、不编造规则。 |
| 单点配置 | 全部规则参数集中在 `gungnir/config.py`；LLM 端点经 env / `.env` 注入。 |
...
| 层 | 职责 | 状态 |
|----|------|------|
| L0 规则引擎 | 状态机 + 37 行有序现金流仿真 + 可行性校验 + 七指标评分投影（信任锚）。 | 已完成 |
| L1 状态采集 | 结构化录入 / 解析每期输入，重建完整公司状态。 | 已完成 |
| L2 需求模型 | 参数化需求函数（价格 / 广告 / 促销 / 等级），预留历史标定接口。 | 已完成 |
| L3 优化器 | 多期贴现目标 + 坐标上升定价；激活研发 / 买机器 / 国债 / 分红 / 批量折扣等长期杠杆。 | 已完成 |
| L4 LLM 层 | 解释「为什么」、管理教学；无密钥时降级为确定性模板。 | 已完成 |
| L5 UI | 单页界面：录入 -> 提案 -> 调整 -> 导出。 | 已完成 |
| M6 复盘评估 | 多期回放、自博弈、评分曲线。 | 已完成 |
...
完整跑通「**录入 -> 提案 -> 调整 -> 导出**」闭环：录入状态 -> 一键提案 / 优化 -> 编辑决策即时重新校验 -> 导出 CSV 决策单。
...
- 2 产品（A/B）x 3 市场 x 10 公司；1 期 = 1 季度；难度 5A。
- 初始现金 2,500,000 元；最低现金 2,000,000 元；信用总额 8,000,000 元。
- 单件资源：A（机器 100 时 / 人力 150 时 / 原料 300）；B（机器 200 时 / 人力 250 时 / 原料 1500）。
- 评分：七项加权 Z-score（本期利润 .20 / 净资产 .20 / 市场份额 .15 / 资本利润率 .15 / 累计分红 .10 / 累计缴税 .10 / 人均利润率 .10）。
...
- **M0 项目脚手架**（已完成）—— repo 结构、依赖、集中 config、README、LICENSE、.gitignore。
- **M1 引擎核心**（已完成）—— `GameState` + `Decision` + 现金流仿真 + 可行性校验 + 评分投影（对拍 `决策工具.xls`）。
- **M2 决策提案**（已完成）—— 给定状态生成一套可行决策（规则 / 简单搜索，不接 LLM）。
- **M3 优化器**（已完成）—— 确定性坐标上升（定价）+ 多期贴现目标，激活国债 / 分红 / 批量折扣长期杠杆。
- **M4 LLM 层**（已完成）—— 接入 DeepSeek（OpenAI 兼容，端点 / 模型经 env 注入），解释 + 教学；无密钥降级。
- **M5 UI**（已完成）—— 单页 HTML/JS + FastAPI，跑通「录入 -> 提案 -> 调整 -> 导出」闭环。
- **M6 复盘 / 评估**（已完成）—— 多期回放、自博弈（N 公司并行）、评分曲线。
...
平台尚有少量规则参数未锁定，在 [gungnir/config.py](gungnir/config.py) 中以 `TODO(待确认)` 标记，完整讨论见 [docs/rules.md](docs/rules.md)：

- L2 需求模型为工作用占位模型（平台不公开需求系数）；可行性不依赖其取值。
- 机器分班产能（2 x 520 时模型）、特殊班工资映射、废品率、交税信用语义。
- 债券摊还、库存变化账面值、评分滞后与 NPV 折算。

各项将随数据可得，逐项与参考决策工具（`决策工具.xls`）对拍锁定。
